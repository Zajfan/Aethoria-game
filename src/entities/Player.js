...
export class Player extends Phaser.Physics.Arcade.Sprite {
  ...
  update(time, delta) {
    ...
    if (usingKeys) {
      // Keyboard overrides everything; clear move target
      if (vx !== 0 && vy !== 0) { vx *= 0.707; vy *= 0.707; }
      this.setVelocity(vx, vy);
      this.moveTarget = null;
    } else if (this.moveTarget) {
      // Click-to-move: direct velocity calculation (never use physics.moveTo — it causes drift)
    ...
    if (this.attackTarget && !this.attackTarget.isDead && this.attackCooldown === 0) {
      ...
      if (dist <= this.attackRange) {
        ...
        // Walk toward target only if not actively using keyboard
        const dx  = this.attackTarget.x - this.x;
        const dy  = this.attackTarget.y - this.y;
        const len = Math.hypot(dx, dy);
        this.setVelocity((dx / len) * sp * 0.85, (dy / len) * sp * 0.85);
      }
    }
  }
